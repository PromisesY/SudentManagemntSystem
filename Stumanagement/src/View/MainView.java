...
public class MainView extends JFrame {
	...
	private void find() {
		...
		String namex = "^[A-Za-z\u4e00-\u9fa5]+$";
		String numderx = "^[A-Za-z\0-9]+$";
		if(param.matches(namex)) {
			String[][] result = ((StudentDAO) BaseDAO.getAbilityDAO(DAO.StudentDAO)).queryByName(param);
			initJTable(MainView.jTable, result);
		       
		}
		else if(param.matches(numderx))
		{
			String[][] result = ((StudentDAO) BaseDAO.getAbilityDAO(DAO.StudentDAO)).queryBySnumber(param);
		       initJTable(MainView.jTable, result);
		}
		condition.setText("");
		//initJTable(MainView.jTable, result);
